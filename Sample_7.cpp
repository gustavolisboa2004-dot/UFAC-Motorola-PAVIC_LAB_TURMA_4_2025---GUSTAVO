...
__global__ void gpu_grayscale(int width, int height, float *image, float *image_out)
{
    ...
    if (x < width && y < height)
    {
        int offset_out = y * width + x;
        int offset_in  = offset_out * 3;

        // BGR format
        float b = image[offset_in + 0];
        float g = image[offset_in + 1];
        float r = image[offset_in + 2];
...
__global__ void gpu_sobel(int width, int height, float *image, float *image_out)
{
    ...
    // Verifica se está dentro dos limites da imagem (considerando a borda de 1px do filtro 3x3)
...
int main(int argc, char **argv)
{
    ...
    // Step 2: Apply a 3x3 Gaussian filter
    {
        ...
        cudaDeviceSynchronize(); // IMPORTANTE para o tempo ser real
        checkCUDAError();
